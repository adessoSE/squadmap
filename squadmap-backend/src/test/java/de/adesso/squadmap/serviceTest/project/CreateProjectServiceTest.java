...
class CreateProjectServiceTest {
    ...
    void checkIfCreateProjectCreatesAProject() {
        ...
        Mockito.when(projectRepository.existsByTitle(title)).thenReturn(false);
        Mockito.when(projectRepository.save(project)).thenReturn(project);
        Mockito.when(projectMapper.map(command)).thenReturn(project);
    ...
    void checkIfCreateProjectThrowsExceptionWhenTitleAlreadyExists() {
        ...
        CreateProjectCommand command = new CreateProjectCommand();
        command.setTitle(title);
        Mockito.when(projectRepository.existsByTitle(title)).thenReturn(true);
